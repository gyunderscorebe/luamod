...
function onPlayerRoleChange(cn, new_role, hash, pwd, isconnect)
    if sdbs.cn:on_role_change(cn, new_role, hash, pwd, isconnect) == PLUGIN_BLOCK then
        return PLUGIN_BLOCK
    end
end
...
function onPlayerSayText(cn,text,team,me)
    if sdbs.cn:on_say_text(cn,text,team,me) == PLUGIN_BLOCK then
        return PLUGIN_BLOCK
    end
end
